...
#let sjtu-header(self) = {
  ...
  if self.store.navigation == "sidebar" {
      ...
      grid(
        ...
        [
          ...
          #align(left + horizon, utils.display-current-heading(
            depth: if last-heading-label == "touying:hidden" {
              self.slide-level - 1
            } else { self.slide-level },
            style: none,
          ))
        ],
        align(right + horizon, image("vi/sjtu-vi-sjtugate.png", height: 0.9cm)),
      ),
      align(center + top, line(length: 100%, stroke: (
    ...
    grid(
      ...
      align(center + horizon, line(length: 100%, stroke: (
        paint: self.colors.primary,
        thickness: 1.5pt,
      ))),
      v(1em),
...
#let title-slide(
  ...
  self = utils.merge-dicts(
    ...
    config-page(
      header: align(right + horizon, block(
        inset: (right: 0.3em, top: 0.3em),
        image("vi/sjtu-vi-logo.png"),
      )),
...
#let title-slide-red(
  ...
  let body = {
    ...
    line(length: 100%, stroke: (
      paint: self.colors.neutral-light,
      thickness: 1.5pt,
    ))
    v(-1.15em)
    block(fill: self.colors.primary, width: 100%, {
      ...
      grid(
        ...
        align(right, block(
          inset: (x: 2em),
          image("vi/sjtu-vi-logo-white.png"),
        )),
      )
      v(2em)
    })
...
#let outline-slide(
  ...
  self = utils.merge-dicts(
    self,
    config-common(freeze-slide-counter: true),
    config-page(footer: sjtu-footer, margin: (
      top: 3em,
    )),
  )
  touying-slide(self: self, config: config, components.adaptive-columns(
    start: text(
      ...
      fill: self.colors.primary,
      weight: "bold",
      utils.call-or-display(self, title) + v(0.5em),
    ),
    text(fill: self.colors.neutral-darkest, outline(
      title: none,
      indent: 1em,
              ...
              if hd.level == 2 {
                ...
                if slide.location().page() <= current-page {
                  [#link(slide.location(), text(
                    size: .7em,
                    v(0em)
                      + box(height: 0.8em, sym.circle.filled)
                      + "  "
                      + hd.body,
                  ))]
                } else {
                  [#link(slide.location(), text(
                    size: .7em,
                    v(0em) + box(height: 0.8em, sym.circle) + "  " + hd.body,
                  ))]
                }
              }
            }
...
#let new-section-slide(
  ...
  self = utils.merge-dicts(self, config-page(
    header: grid(
      ...
      grid(
        columns: (1fr, 35%),
        align(left + horizon, text(size: 1.9em, utils.display-current-heading(
          depth: self.slide-level,
          style: auto,
        ))),
  ...
  show: touying-slides.with(
    config-page(
      ...
      margin: if navigation == "sidebar" {
        ...
        (
          top: if mini-slides.linebreaks { mini-slides.height } else { 6em },
          bottom: 3em,
          x: mini-slides.x,
        )
      } else {
...
#let iconlink(
  ...
  [#box(fa-icon(icon, solid: solid), inset: (right: 3pt))#link(uri)[#text]]
}

#let githublink(uri, text: "") = iconlink(uri, text: text, icon: "github")
